...
# -----------------------------------------------------------------------------
# LDONet-S 直接训练脚本（PolyU）
#
# 说明：
# 1) 该脚本调用 NewTrain/train_LDONet_S.py。
# 2) 适用于 Windows + Git Bash / WSL / Linux。
# 3) 默认使用 dataset/train_PolyU.txt。
# -----------------------------------------------------------------------------

usage() {
  cat <<'EOF'
用法:
  bash NewTrain/scripts/train/train_PolyU_LDONet_S.sh [选项]
...
选项:
  ...
  --lr <v>                学习率（默认: 0.001）
  --weight-decay <v>      AdamW 权重衰减（默认: 0.005）
  --warmup-epochs <n>     预热轮数（默认: 10）
  --min-lr <v>            最小学习率（默认: 1e-6）
  --save-interval <n>     保存间隔（默认: 5）
  --train-file <path>     训练索引文件（默认: dataset/train_PolyU.txt）
  --student-path <path>   学生模型恢复训练 checkpoint（可选）
  --help                  显示帮助

示例:
  bash NewTrain/scripts/train/train_PolyU_LDONet_S.sh --gpu 0 --epochs 200
EOF
}
...
TRAIN_SET_FILE="${PROJECT_ROOT}/dataset/train_PolyU.txt"

DES_PATH="${PROJECT_ROOT}/results/PolyU/LDONet_S/checkpoint/"
PATH_RST="${PROJECT_ROOT}/results/PolyU/LDONet_S/rst_test/"
RUN_ID_NUM="193"
