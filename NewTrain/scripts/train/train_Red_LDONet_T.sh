...
# LDONet-T 教师模型训练脚本（Red）

usage() {
  cat <<'EOF'
用法:
  bash NewTrain/scripts/train/train_Red_LDONet_T.sh [选项]
...
选项:
  ...
  --train-file <path>  训练索引文件（默认: dataset/train_Red.txt）
  --help               显示帮助
EOF
}
...
TRAIN_SET_FILE="${PROJECT_ROOT}/dataset/train_Red.txt"

DES_PATH="${PROJECT_ROOT}/results/Red/LDONet_T/checkpoint/"
PATH_RST="${PROJECT_ROOT}/results/Red/LDONet_T/rst_test/"
RUN_ID_NUM="250"
