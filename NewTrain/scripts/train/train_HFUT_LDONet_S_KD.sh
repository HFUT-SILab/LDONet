...
if [[ -z "${TRAIN_FILE}" ]]; then
    TRAIN_FILE="${PROJECT_ROOT}/dataset/train_HFUT.txt"
fi

if [[ -z "${TEACHER_PATH}" ]]; then
    TEACHER_PATH="${PROJECT_ROOT}/results/HFUT/LDONet_T/checkpoint/net_params_best.pth"
fi

DES_PATH="${PROJECT_ROOT}/results/HFUT/LDONet_S_KD/checkpoint/"
PATH_RST="${PROJECT_ROOT}/results/HFUT/LDONet_S_KD/rst_test/"
...
if [[ ! -f "${TEACHER_PATH}" ]]; then
    ...
    echo "请先用 train_HFUT_LDONet_T.sh 训练教师模型，或通过 --teacher-path 指定路径。" >&2
    exit 1
fi
...
echo "======================================================================"
echo "   LDONet-T -> LDONet-S 知识蒸馏训练（HFUT）"
echo "======================================================================"
...
"${PYTHON_EXE}" "${TRAIN_SCRIPT_P}" \
    ...
    --des_path "${DES_PATH_P}" \
    --path_rst "${PATH_RST_P}" \
    --run_dir run_kd_LDONet_S_KD_HFUT
