...
# -----------------------------------------------------------------------------
# LDONet-S 直接训练脚本（Tongji）
#
# 说明：
# 1) 该脚本调用 NewTrain/train_LDONet_S.py。
# 2) 适用于 Windows + Git Bash / WSL / Linux。
# 3) 默认使用 dataset/train_Tongji.txt。
# -----------------------------------------------------------------------------

usage() {
  cat <<'EOF'
用法:
  bash NewTrain/scripts/train/train_Tongji_LDONet_S.sh [选项]
...
示例:
  bash NewTrain/scripts/train/train_Tongji_LDONet_S.sh --gpu 0 --epochs 200
EOF
}
...
DES_PATH="${PROJECT_ROOT}/results/Tongji/LDONet_S/checkpoint/"
PATH_RST="${PROJECT_ROOT}/results/Tongji/LDONet_S/rst_test/"
RUN_ID_NUM="300"
...
if [[ -n "${STUDENT_PATH}" ]]; then
  CMD+=(--student_path "$(to_win_path "${STUDENT_PATH}")")
fi
