...
# -----------------------------------------------------------------------------
# LDONet-S 直接训练脚本（Blue）
# -----------------------------------------------------------------------------

usage() {
  cat <<'EOF'
用法:
  bash NewTrain/scripts/train/train_Blue_LDONet_S.sh [选项]
...
选项:
  ...
  --train-file <path>     训练索引文件（默认: dataset/train_Blue.txt）
  --help                  显示帮助
EOF
}
...
TRAIN_SET_FILE="${PROJECT_ROOT}/dataset/train_Blue.txt"

DES_PATH="${PROJECT_ROOT}/results/Blue/LDONet_S/checkpoint/"
PATH_RST="${PROJECT_ROOT}/results/Blue/LDONet_S/rst_test/"
RUN_ID_NUM="250"
