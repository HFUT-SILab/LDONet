...
#     '''
#
#     def __init__(self, txt, transforms=None, train=True, imside=256, outchannels=1):
#
#         self.train = train
#
...
#                     T.ToTensor(),
#                     NormSingleROI(outchannels=self.chs)  # 归一化图像
#
#                     ])
#             else:
#                 self.transforms = T.Compose([
...
class MyDataset(data.Dataset):
        ...
        if transforms is None:
            if not train:
                ...
                self.transforms = T.Compose([
                    ...
                    T.Resize(self.imside),
                    # T.RandomChoice(transforms=[
                    #     T.ColorJitter(brightness=0.1, contrast=0, saturation=0, hue=0),  # 0.3 0.35
                    #     T.RandomResizedCrop(size=self.imside, scale=(0.8, 1.0), ratio=(1.0, 1.0)),
                    #     T.RandomPerspective(distortion_scale=0.15, p=1),  # (0.1, 0.2) (0.05, 0.05)
                    #     # T.RandomPerspective(distortion_scale=0.1, p=0.2),
                    #     T.RandomChoice(transforms=[
                    #         T.RandomRotation(degrees=10, resample=Image.BICUBIC, expand=False,
                    ...
                    T.RandomChoice(transforms=[
                        ...
                        # T.RandomPerspective(distortion_scale=0.1, p=0.2),
                        T.RandomChoice(transforms=[
                            T.RandomRotation(degrees=10, interpolation=InterpolationMode.BICUBIC, expand=False,
                                             center=(0.5 * self.imside, 0.0)),
                            T.RandomRotation(degrees=10, interpolation=InterpolationMode.BICUBIC, expand=False,
                                             center=(0.0, 0.5 * self.imside)),
                        ]),
    ...
    def __getitem__1(self, index):
        ...
        # 获取任意一张与当前样本不同类别的图片的数据
        random_index = np.random.choice(np.arange(len(self.images_label))[np.array(self.images_label) != label])
        img_path3 = self.images_path[random_index]
        data3 = Image.open(img_path3).convert('L')
